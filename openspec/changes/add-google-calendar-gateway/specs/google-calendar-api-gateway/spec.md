...
#### Scenario: Find UID globally across configured Google calendars
- **WHEN** SmartPlanner looks up an event UID using the Google provider
- **THEN** the gateway SHALL search every configured Google calendar by iCalendar UID and SHALL return null only if no configured calendar contains that UID

#### Scenario: Duplicate UID fails safely
- **WHEN** more than one configured Google calendar contains the same iCalendar UID
...
- **THEN** the gateway SHALL create or update only the corresponding Google event and SHALL preserve its iCalendar UID for future lookup
